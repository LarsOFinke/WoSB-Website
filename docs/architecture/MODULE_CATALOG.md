...
| `spring-api/src/main/java/eu/royalblackwater/api/warehouse/` | Member-readable, staff-managed fleet stock with member-linked or custom holders, administrator-managed game ports, collection state, fleet-port pickup assignees, reservation state, optimistic concurrency, and audit-backed notifications. Discord remains a downstream subscriber rather than a persistence dependency. | `WarehouseServiceTest`, `WarehousePortServiceTest`, `WarehousePortAssignmentServiceTest`, warehouse HTTP lifecycle integration, migrations V12–V14, and [warehouse reference](../reference/GUILD_WAREHOUSE.md); inspect holder membership, active port, assignment membership, and row version on 400/409 responses. |
...
| `frontend/src/modules/privacy/` | Privacy center and cookie banner; without a saved decision it opens automatically, and failed initialization keeps settings controls visible in a fail-closed state. | `cookieConsentVisibility.test.mjs` and browser smoke tests for retry, payload, and error state. |
...
| `frontend/src/modules/warehouse/` | Member warehouse module with spreadsheet-style facets and totals plus active port dropdowns, collection state, fleet-port pickup assignments, staff-only member/custom holder editing, reservation state, and conflict-safe mutations. | `warehouse.test.mjs`, member/staff browser coverage, `useWarehousePage`, and backend 409 handling. |
