...
| `spring-api/src/main/java/eu/royalblackwater/api/warehouse/` | Member-readable, staff-managed fleet stock with member-linked or custom holders, administrator-managed game ports, reservation state, optimistic concurrency, and audit-backed notifications. Discord remains a downstream subscriber rather than a persistence dependency. | `WarehouseServiceTest`, `WarehousePortServiceTest`, warehouse HTTP lifecycle integration, migrations V12–V13, and [warehouse reference](../reference/GUILD_WAREHOUSE.md); inspect holder membership, active port, and row version on 400/409 responses. |
...
| `frontend/src/modules/warehouse/` | Member warehouse module with spreadsheet-style facets and totals plus active port dropdowns, staff-only member/custom holder editing, reservation state, and conflict-safe mutations. | `warehouse.test.mjs`, member/staff browser coverage, `useWarehousePage`, and backend 409 handling. |
