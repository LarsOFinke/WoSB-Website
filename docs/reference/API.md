...
The versioned OpenAPI 3.1 authoring source under `openapi/source/` is the
...
Every `/api/` response carries an `X-Request-Id` generated by the backend. The
same identifier is included in central `api_error`, `security_401` and
`security_403` log records so test automation and operators can correlate a
response with the bounded server-side diagnostic event. Successful request
lifecycle logging is disabled by default and can be enabled temporarily with
`RBF_HTTP_LIFECYCLE_LOGGING=true`; it records request ID, method, normalized
route, response status and duration only. Payloads, query values, cookies,
client IPs and user agents are never part of this diagnostic stream.
...
1. Change and review the owning files under `openapi/source/`.
2. Run `python3 infrastructure/scripts/generation/assemble_openapi.py`.
3. Regenerate API DTOs with
   `python3 infrastructure/scripts/generation/generate_api_dtos.py`; never edit
   generated DTO records manually.
4. Update the owning module controller mapping/bindings and run
   `python3 infrastructure/scripts/quality/audit_controller_contract.py`.
5. Regenerate `API_ENDPOINTS.md` with
   `python3 infrastructure/scripts/generation/generate_api_reference.py`.
6. Update services, mappers/repositories, authorization, frontend API modules and
