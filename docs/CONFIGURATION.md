...
- `backend/.env` must exist, or `RBF_ENV_FILE` must point to another env file.
- `backend/config/app.toml` must exist, or `RBF_CONFIG_FILE` must point to another config file.
...
```env
APP_ENV=production
DATABASE_URL=postgresql+psycopg://rbf:<secret>@postgres:5432/rbf
...
SEED_ADMIN_DISPLAY_NAME=RBF Command
```
