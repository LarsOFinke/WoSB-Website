...
[[ -f "$REQUEST" && -n "$HOST" && -n "$RESULT" ]] || {
  echo "--request, --host und --result sind erforderlich." >&2
  exit 2
}
...
[[ -n "$OPERATOR_HOME" && "$OPERATOR_HOME" == /* ]] || { echo "Operator-Home konnte nicht ermittelt werden." >&2; exit 1; }
...
if [[ -z "$RECOVERY_PUBLIC_KEY" ]]; then
  ...
  [[ ! -e "$RECOVERY_KEY" && ! -e "$RECOVERY_KEY.pub" ]] || { echo "Recovery-Leseschlüssel existiert bereits: $RECOVERY_KEY" >&2; exit 1; }
...
if not re.fullmatch(r"(ssh-ed25519|ssh-rsa|ecdsa-sha2-nistp(?:256|384|521)) [A-Za-z0-9+/=]+(?: [^\r\n]{1,128})?",value):
    raise SystemExit("Ungültiger Recovery-Public-Key.")
PY
AGE_IDENTITY="$RECOVERY_DIR/rbf-recovery-identity.txt"
[[ ! -e "$AGE_IDENTITY" ]] || { echo "age-Identität existiert bereits: $AGE_IDENTITY" >&2; exit 1; }
...
if payload.get("managed_by") != "rbf-backup-server-provisioner":
    raise SystemExit("Vorhandener Status stammt nicht vom RBF Backup Server Provisioner.")
PY
else
...
echo "Private Recovery-Dateien: ${RECOVERY_DIR} (zusätzlich verschlüsselt offline sichern)"
