...
def main(argv: list[str] | None = None) -> int:
    ...
    recovery = sub.add_parser(
        "recovery", help="Vollständige Wiederherstellbarkeit nachweisen"
    )
    recovery_sub = recovery.add_subparsers(dest="recovery_action", required=True)
    recovery_verify = recovery_sub.add_parser(
        "verify", help="Import, Migration, Schlüssel und API-Readiness prüfen"
    )
    recovery_verify.add_argument("bundle", type=Path)
    recovery_verify.add_argument("--identity", type=Path, required=True)
    recovery_verify.add_argument("--repository", type=Path, required=True)
    recovery_verify.add_argument("--report", type=Path, required=True)
    recovery_verify.add_argument("--allow-legacy", action="store_true")
    recovery_verify.add_argument("--allow-uncoordinated", action="store_true")
    ...
    server = sub.add_parser(
        "server", help="Dedizierten Backup-Server automatisch provisionieren"
    )
    ...
    provision.add_argument(
        "request", type=Path, help="Von der WoSB-Webseite geladene Enrollment-Anfrage"
    )
    provision.add_argument(
        "--host",
        required=True,
        help="Vom Produktivserver erreichbare IP oder DNS-Adresse",
    )
    provision.add_argument(
        "--output", type=Path, required=True, help="Zu erzeugende RESPONSE.json"
    )
    provision.add_argument(
        "--identity",
        type=Path,
    ...
    provision.add_argument(
        "--port", type=int, default=22, help="Bereits konfigurierter SSH-Port"
    )
    provision.add_argument(
        "--user", default="rbf-backup", help="Upload-Konto aus der Enrollment-Anfrage"
    )
    provision.add_argument(
        "--recovery-user",
        default="rbf-recovery",
        help="Lokales read-only Recovery-Konto",
    )
    provision.add_argument(
        "--recovery-key",
        type=Path,
    ...
    provision.add_argument(
        "--allow-from", default="", help="Optionale Produktivserver-IP/CIDR"
    )
    ...
    lab.add_argument("--report", type=Path)
    ...
    if args.command == "recovery":
        result = verify_recovery(
            ...
            allow_uncoordinated=args.allow_uncoordinated,
        )
        print(
            f"recoverable={str(result.recoverable).lower()} compatibility={result.compatibility} report={result.report}"
        )
        return 0
    if args.command == "timer":
        install_pull_timer(
            args.calendar
        ) if args.action == "install" else remove_pull_timer()
        return 0
    if args.command == "server":
        ...
        print(
            f"SSH-Host-Key-Fingerprint: {response.get('host_key_fingerprint', 'unbekannt')}"
        )
        print(f"Private age-Identität: {args.identity.expanduser().resolve()}")
        print(
            f"Lokaler Recovery-Leseschlüssel: {args.recovery_key.expanduser().resolve()}"
        )
    ...
    if args.command == "lab":
        if args.action == "init":
            ...
            if not args.bundle or not args.identity:
                raise RuntimeError(
                    f"lab {args.action} benötigt --bundle und --identity."
                )
            if args.action == "restore":
                print(restore_bundle(args.bundle, args.identity).safe_summary)
            else:
                if not args.report:
                    raise RuntimeError("lab import-check benötigt --report.")
                result = import_check_bundle(args.bundle, args.identity, args.report)
                print(f"recoverable=false import_ok=true report={result.report}")
