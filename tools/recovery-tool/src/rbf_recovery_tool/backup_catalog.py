...
def _inspect_set(sftp, root: PurePosixPath, attribute, by_name: dict[str, object]) -> BackupCatalogEntry:
    filename = str(attribute.filename)
    created_at = _fallback_time(getattr(attribute, "st_mtime", None))
    reason = "unknown"
    try:
        payload = json.loads(_verified_remote_file(sftp, root, filename, limit=_MAX_SET_BYTES).decode("utf-8"))
        ...
        for artifact_type, record in artifacts.items():
            ...
            sidecar = _remote_bytes(sftp, root / f"{artifact_name}.sha256", limit=4096)
        ...
        if isinstance(verification, dict):
            ...
            if not _REPORT_RE.fullmatch(report_name):
                raise RuntimeError("verification report filename is unsupported")
            report = json.loads(_verified_remote_file(sftp, root, report_name, limit=_MAX_REPORT_BYTES).decode("utf-8"))
            if not isinstance(report, dict) or report.get("schema_version") != 2 or report.get("mode") != "preflight":
                raise RuntimeError("verification report is not a Spring preflight")
            checks = report.get("checks")
            passed = {
                str(check.get("name")) for check in checks
                if isinstance(check, dict) and check.get("status") == "passed"
            } if isinstance(checks, list) else set()
            recoverable = report.get("status") == "passed" and report.get("recoverable") is True and {
        ...
        if "recovery" not in artifacts:
            raise RuntimeError("recovery bundle is not included")
        return BackupCatalogEntry(
            ...
            detail="Committed backup set and Spring/Flyway preflight verified.",
        )
    except (KeyError, OSError, TypeError, ValueError, UnicodeDecodeError, json.JSONDecodeError, RuntimeError) as exc:
        return BackupCatalogEntry(
            filename=filename,
            created_at=created_at,
            reason=reason,
...
def fetch_backup_catalog(profile: Profile, *, password: str = "") -> list[BackupCatalogEntry]:
