...
def initialize_lab(port: int = _DEFAULT_PORT) -> LabConnection:
    if not 1024 <= int(port) <= 65535:
        raise ValueError(
            "Der lokale PostgreSQL-Port muss zwischen 1024 und 65535 liegen."
        )
    root = lab_root()
...
def connection() -> LabConnection:
    ...
    return LabConnection(
        "127.0.0.1",
        int(values["POSTGRES_LOCAL_PORT"]),
        values["POSTGRES_DB"],
...
def lab_status() -> LabStatus:
    ...
    return LabStatus(
        ...
        running,
        healthy,
        "Gesund" if healthy else ("Gestartet" if running else "Gestoppt"),
    )
...
def start_lab() -> LabConnection:
    ...
    if not docker_available():
        raise RuntimeError(
            "Docker ist nicht erreichbar. Richte zuerst rootless Docker ein."
        )
...
def _exec_postgres(*arguments: str, input_stream=None, timeout: int = 600) -> None:
    process = subprocess.Popen(
        _compose_command("exec", "-T", "postgres", *arguments),
        stdin=input_stream,
    ...
    try:
        ...
        process.communicate()
        raise RuntimeError(
            "Der lokale PostgreSQL-Vorgang hat das Zeitlimit überschritten."
        )
    if process.returncode != 0:
        detail = (
            (stderr or stdout or b"")
            .decode("utf-8", errors="replace")
            .strip()
            .splitlines()
        )
        raise RuntimeError(
            f"Lokaler PostgreSQL-Vorgang fehlgeschlagen{f' ({detail[-1]})' if detail else ''}."
        )
...
def import_check_bundle(
    bundle: Path, identity: Path, report_path: Path
    ...
    report = new_report(
        mode="import-check",
        source=str(bundle),
        source_artifact=_artifact_binding(bundle),
    )
    try:
        ...
        add_report_check(
            ...
            status="passed",
            detail="Encrypted bundle checksum and manifest inventory are verified during extraction.",
        )
        ...
        add_report_check(
            ...
            status="passed",
            detail="PostgreSQL imported the dump and SELECT 1 succeeded.",
        )
        add_report_check(
            report,
            name="runtime_recovery",
            status="skipped",
            detail="Import checks do not execute migrations or start the application.",
        )
...
def _application_preflight_override(
    image: str, recovered_env: Path, details: LabConnection
) -> str:
...
def verify_recovery(
    ...
    *,
    ...
    if not (backend / "Dockerfile").is_file() or not migrations.is_dir():
        raise RuntimeError(
            "Das angegebene Repository enthält kein vollständiges WoSB-Backend."
        )
    report = new_report(
    ...
    try:
        with tempfile.TemporaryDirectory(prefix="rbf-full-recovery-") as temporary:
            extracted, manifest = extract_verified_bundle(
                bundle, identity, Path(temporary) / "bundle"
            )
            add_report_check(
                report,
                name="bundle_integrity",
                status="passed",
                detail="Encrypted bundle, sidecar, safe paths and manifest inventory are valid.",
            )
            artifacts = (
                manifest.get("artifacts") if isinstance(manifest, dict) else None
            )
            if not isinstance(artifacts, dict):
                raise RuntimeError("Recovery manifest has no artifacts mapping.")
            ...
            if metadata.is_file():
                ...
                assessment = assess_compatibility(
                    descriptor,
                    MigrationGraph.from_directory(migrations),
                    allow_unrecorded=allow_legacy,
                )
                compatibility_status = assessment.status
                if not assessment.compatible:
                    raise RuntimeError(assessment.detail)
                if not is_production_consistent(descriptor) and not allow_uncoordinated:
                    raise RuntimeError(
                        "Das Backup besitzt keinen produktiven Konsistenznachweis."
                    )
                add_report_check(
                    report,
                ...
                add_report_check(
                    ...
                    name="metadata_compatibility",
                    status="warning",
                    detail="Legacy bundle without restore metadata was explicitly accepted.",
                )
                compatibility_status = "legacy"
            else:
                raise RuntimeError(
                    "Restore-Metadaten fehlen; verwende nur im Notfall --allow-legacy."
                )
            details = restore_dump(dump)
            add_report_check(
                report,
                name="staging_database_creation",
                status="passed",
                detail="Rootless isolated PostgreSQL lab is running.",
            )
            add_report_check(
                report,
                name="postgres_import",
                status="passed",
                detail="Dump import and SQL probe succeeded.",
            )
            recovered_env = extracted / "configuration/infrastructure.env"
            if not recovered_env.is_file():
                raise RuntimeError("Recovery bundle has no infrastructure.env.")
            image = f"rbf-recovery-preflight:{hashlib.sha256(str(repository).encode()).hexdigest()[:12]}"
            _run(
                [*_docker_base(), "build", "--pull", "-t", image, str(backend)],
                timeout=3600,
            )
            override = lab_root() / "preflight.compose.yaml"
            override.write_text(
                _application_preflight_override(image, recovered_env, details),
                encoding="utf-8",
            )
            try:
            ...
            _run(
                _compose_command(
                    "run", "--rm", "recovery-migrate", overrides=(override,)
                ),
                timeout=3600,
            )
            add_report_check(
                report,
                name="migration_and_schema_preflight",
                status="passed",
                detail="Alembic upgrade head and alembic check succeeded using the current backend image.",
            )
            add_report_check(
                report,
                name="secret_key_preflight",
                status="passed",
                detail="Encrypted application records are readable with the recovered key ring.",
            )
            _run(
                _compose_command("up", "-d", "recovery-api", overrides=(override,)),
                timeout=300,
            )
            ...
            while time.monotonic() < deadline:
                status = _run(
                    _compose_command(
                        "ps", "--format", "json", "recovery-api", overrides=(override,)
                    ),
                    timeout=30,
                    check=False,
                )
                if "healthy" in (status.stdout or "").casefold():
                    healthy = True
                    break
                time.sleep(3)
            if not healthy:
                logs = _run(
                    _compose_command(
                        "logs", "--no-color", "recovery-api", overrides=(override,)
                    ),
                    timeout=60,
                    check=False,
                )
                raise RuntimeError(
                    f"API readiness failed: {(logs.stdout or logs.stderr or '').strip()[-1000:]}"
                )
            add_report_check(
                report,
                name="application_readiness_preflight",
                status="passed",
                detail="Current API image reached readiness inside an internal network without published ports.",
            )
            _run(
                _compose_command(
                    "rm", "-s", "-f", "recovery-api", overrides=(override,)
                ),
                timeout=120,
                check=False,
            )
            add_report_check(
                ...
                status="passed",
                detail="Temporary application preflight container was removed; lab remains available for inspection.",
            )
        finalize_report(report, status="passed", recoverable=True)
        write_report(report_path, report)
        return RecoveryVerificationResult(
            report_path, True, compatibility_status, details
        )
    except Exception as exc:
        add_report_check(
            report, name="recovery_verify", status="failed", detail=str(exc)
        )
        finalize_report(report, status="failed", recoverable=False)
        write_report(report_path, report)
        raise
