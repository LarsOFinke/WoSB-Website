...
for executable, label in ((age_executable, "age"), (age_keygen_executable, "age-keygen")):
    if not executable.is_file():
        raise SystemExit(f"{label} executable not found: {executable}")

datas = []
binaries = [(str(age_executable), "bin"), (str(age_keygen_executable), "bin")]
hiddenimports = []
...
exe = EXE(
    ...
    console=console_mode, disable_windowed_traceback=False,
)
