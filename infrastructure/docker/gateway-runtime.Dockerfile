...
RUN sed -i "s|__RBF_MAINTENANCE_URL__|${MAINTENANCE_URL}|g" /etc/nginx/conf.d/default.conf
...
RUN sed -i "s|__RBF_MAINTENANCE_URL__|\${MAINTENANCE_URL}|g" /usr/share/nginx/html/maintenance.html
USER 101
ENTRYPOINT ["nginx"]
CMD ["-g", "daemon off;"]
EXPOSE 8080 8443
