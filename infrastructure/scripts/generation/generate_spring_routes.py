...
def generate_api(group: str, operations: list[tuple[str, str, dict]]) -> tuple[str, str]:
    ...
    for path, http_method, operation in operations:
        ...
        signature, _, _, multipart, nested = operation_signature(operation)
        imports.update(nested)
        ...
        if multipart:
            ...
            annotation = f'@{mapping}(value = "{path}", consumes = MediaType.MULTIPART_FORM_DATA_VALUE)'
        else:
            annotation = f'@{mapping}("{path}")'
        methods.append(f"    {annotation}\n" + declaration)
