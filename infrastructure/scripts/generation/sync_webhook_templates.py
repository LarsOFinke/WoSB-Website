...
ROOT=Path(__file__).resolve().parents[3]; SOURCE=ROOT/'spring-api/src/main/reference/webhook-events.json'
def main():
    payload=json.loads(SOURCE.read_text()); events=payload.get('events')
    if payload.get('schema_version')!=1 or not isinstance(events,list):raise SystemExit('Invalid webhook event catalog')
    ...
    sections=['# Discord webhook message templates','','Generated from `spring-api/src/main/reference/webhook-events.json`.']
