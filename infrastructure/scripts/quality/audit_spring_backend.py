...
INFRASTRUCTURE_MODULES = {"config", "dto", "persistence", "shared"}
...
for path in controllers:
    ...
    mappings = re.findall(r"@(Get|Post|Put|Patch|Delete)Mapping\(([^\n]+)\)", source)
    require(mappings, f"controller owns no HTTP mappings: {path.relative_to(ROOT)}")
    route_count += len(mappings)
    for verb, args in mappings:
        match = re.search(r'"([^"]+)"', args)
        require(match is not None, f"controller mapping lacks a literal route: {path.relative_to(ROOT)}")
        route = (verb.lower(), match.group(1))
        require(route not in seen_routes, f"duplicate controller route: {verb.upper()} {route[1]}")
        seen_routes.add(route)
require(route_count == len(ops), f"controllers expose {route_count} routes for {len(ops)} OpenAPI operations")
require(not (JAVA / "contract").exists(),
        "obsolete generated contract layer remains; HTTP bindings belong directly to controllers")
require(not (JAVA / "shared/mapper/ContractConversionService.java").exists(),
        "generic contract conversion service remains; transport conversion belongs to explicit mappers")
...
for path in JAVA.rglob("*.java"):
    ...
    if len(parts) >= 3 and parts[1] in LAYER_NAMES:
        ...
        if layer == "controller":
            ...
            forbidden = [value for value in imported if ".controller." in value or ".contract." in value
...
for path in dto_files:
    source = read(path)
    require(source.startswith("// Generated API DTO by infrastructure/scripts/generation/generate_api_dtos.py"),
...
print(
    f"Spring backend audit OK ({len(ops)} operations, {len(controllers)} controllers, "
    f"{len(list(JAVA.rglob('*.java')))} Java files)."
)
