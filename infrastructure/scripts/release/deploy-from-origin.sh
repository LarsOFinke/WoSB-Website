...
cleanup_command=(sudo bash "$remote_dir/cleanup-failed-release.sh" --if-present --replace-active --yes)
...
remote_command+=(--artifact "$remote_dir/$(basename "$artifact")" --checksum "$remote_dir/$(basename "$checksum")" --skip-backup --no-backup)
