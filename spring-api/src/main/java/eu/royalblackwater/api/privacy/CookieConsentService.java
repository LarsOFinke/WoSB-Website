...
import eu.royalblackwater.api.contract.CookieConsentPolicy;
import eu.royalblackwater.api.contract.CookieConsentRead;
...
public class CookieConsentService {
    ...
    public CookieConsentPolicy policy() {
        return new CookieConsentPolicy(CATEGORIES, POLICY_VERSION);
    }
    ...
    public CookieConsentRead state(HttpServletRequest request) {
        ...
        return repository.findFirstByConsentKeyOrderByCreatedAtDescIdDesc(key)
                ...
                .map(row -> new CookieConsentRead(
                        row.isAnalytics(), row.getCreatedAt(), row.isExternalMedia(), true,
                        true, row.getPolicyVersion(), row.isPreferences()))
    ...
    public SavedConsent save(CookieConsentChoice choice, HttpServletRequest request, AuthenticatedUser user) {
        ...
        CookieConsentEntity saved = repository.save(new CookieConsentEntity(
                ...
                now()));
        CookieConsentRead read = new CookieConsentRead(
                saved.isAnalytics(), saved.getCreatedAt(), saved.isExternalMedia(), true,
                true, saved.getPolicyVersion(), saved.isPreferences());
    ...
    private CookieConsentRead empty() {
        return new CookieConsentRead(false, null, false, false, true, POLICY_VERSION, false);
    }
