...
import eu.royalblackwater.api.security.SecretBoxException;
...
public class RaidHelperHttpClient {
    ...
    public Response request(Map<String, Object> profile, String method, String path, Map<String, Object> payload) {
        if (!path.startsWith("/") || path.contains("..")) {
            throw new RaidHelperTransportException("Invalid Raid-Helper request path.");
        }
        String apiKey;
        try {
            apiKey = normalizeKey(secrets.decrypt(requiredString(profile, "api_key_encrypted")));
        ...
        HttpRequest request = HttpRequest.newBuilder(URI.create(policy.baseUrl(requiredString(profile, "api_base_url")) + path))
