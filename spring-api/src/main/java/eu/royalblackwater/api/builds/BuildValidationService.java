...
class BuildValidationService {
    ...
    BuildValidationService(BuildCatalogRepository catalog, JdbcQueryService jdbc, BuildEffectService effects,
    ...
    BuildPreparedPayload prepare(BuildPayload payload) {
        if (jdbc.count("select count(*) from build_roles where slug=:slug", Map.of("slug", payload.type())) == 0) {
