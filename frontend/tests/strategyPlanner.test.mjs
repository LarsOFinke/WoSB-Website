...
const toolbarSource = await readFile(new URL('../src/modules/strategy-planner/components/command-deck/StrategyToolbar.vue', import.meta.url), 'utf8')
const setupDeckSource = await readFile(new URL('../src/modules/strategy-planner/components/command-deck/StrategySetupDeck.vue', import.meta.url), 'utf8')
const backgroundControlsSource = await readFile(new URL('../src/modules/strategy-planner/components/command-deck/StrategyBackgroundControls.vue', import.meta.url), 'utf8')
const canvasToolsSource = await readFile(new URL('../src/modules/strategy-planner/components/canvas-tools/StrategyCanvasTools.vue', import.meta.url), 'utf8')
const markerToolsSource = await readFile(new URL('../src/modules/strategy-planner/components/canvas-tools/StrategyMarkerTools.vue', import.meta.url), 'utf8')
const managementSource = await readFile(new URL('../src/modules/strategy-planner/components/management/StrategyManagement.vue', import.meta.url), 'utf8')
const markerSectionSource = await readFile(new URL('../src/modules/strategy-planner/components/marker-deck/StrategyMarkerSection.vue', import.meta.url), 'utf8')
const selectionSectionSource = await readFile(new URL('../src/modules/strategy-planner/components/inspector/StrategySelectionSection.vue', import.meta.url), 'utf8')
const sharingSectionSource = await readFile(new URL('../src/modules/strategy-planner/components/management/StrategySharingSection.vue', import.meta.url), 'utf8')
const transformSectionSource = await readFile(new URL('../src/modules/strategy-planner/components/inspector/StrategyTransformSection.vue', import.meta.url), 'utf8')
const objectEditorSource = await readFile(new URL('../src/modules/strategy-planner/components/inspector/StrategyObjectEditor.vue', import.meta.url), 'utf8')
const canvasSource = await readFile(new URL('../src/modules/strategy-planner/components/canvas/StrategyCanvas.vue', import.meta.url), 'utf8')
...
test('strategy info, drawing, canvas tools and management remain separate', () => {
  assert.equal(toolbarSource.match(/<details\b/g)?.length, 4)
  const canvasSectionSources = [markerSectionSource, selectionSectionSource, transformSectionSource]
  assert.equal(canvasSectionSources.join('\n').match(/<details\b/g)?.length, 3)
  assert.match(setupDeckSource, /strategy-title-field/)
  assert.match(setupDeckSource, /strategy-description-field/)
  assert.match(setupDeckSource, /strategy-setup-heading/)
  assert.match(setupDeckSource, /StrategyBackgroundControls/)
  assert.match(backgroundControlsSource, /strategy-background-adjustments/)
  assert.doesNotMatch(toolbarSource, /StrategyBriefingSection/)
  assert.match(toolbarSource, /strategy-command-sections/)
  assert.match(canvasToolsSource, /strategy-canvas-tools/)
  assert.doesNotMatch(canvasToolsSource, /StrategyMarkerSection/)
  assert.match(canvasToolsSource, /StrategySelectionSection/)
  assert.match(canvasToolsSource, /StrategyTransformSection/)
  assert.match(markerToolsSource, /strategy-marker-tools/)
  assert.match(markerToolsSource, /StrategyMarkerSection/)
  assert.doesNotMatch(markerToolsSource, /StrategySelectionSection|StrategyTransformSection/)
  assert.match(managementSource, /strategy-management-panel/)
  assert.match(managementSource, /StrategySharingSection/)
  assert.doesNotMatch(managementSource, /StrategySelectionSection|StrategyTransformSection/)
  assert.match(sharingSectionSource, /viewPublicStrategy/)
  assert.match(selectionSectionSource, /strategy-selection-section/)
  assert.match(transformSectionSource, /strategy-transform-section/)
  assert.doesNotMatch(selectionSectionSource, /type="range"/)
  assert.match(objectEditorSource, /strategy-text-color-field/)
  assert.match(plannerPageSource, /StrategyCanvasTools/)
  assert.match(plannerPageSource, /StrategyMarkerTools/)
  assert.match(plannerPageSource, /StrategyManagement/)
  assert.match(plannerPageSource, /components\/canvas\/StrategyDocument/)
  assert.match(plannerPageSource, /components\/command-deck\/StrategySetupDeck/)
  assert.match(plannerPageSource, /components\/command-deck\/StrategyToolbar/)
  assert.match(plannerPageSource, /components\/canvas-tools\/StrategyCanvasTools/)
  assert.match(plannerPageSource, /components\/canvas-tools\/StrategyMarkerTools/)
  assert.match(plannerPageSource, /components\/management\/StrategyManagement/)
  assert.match(plannerPageSource, /aria-controls="strategy-canvas-tools"/)
  assert.match(plannerPageSource, /aria-controls="strategy-marker-tools"/)
  assert.match(managementSource, /strategy-management-toggle/)
  assert.match(plannerPageSource, /canvasToolsOpen/)
  assert.match(plannerPageSource, /markerToolsOpen/)
  assert.match(plannerPageSource, /managementOpen/)
})
