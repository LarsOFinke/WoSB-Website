...
const inspectorSource = await readFile(new URL('../src/modules/strategy-planner/components/inspector/StrategyInspector.vue', import.meta.url), 'utf8')
const markerOverlaySource = await readFile(new URL('../src/modules/strategy-planner/components/marker-deck/StrategyMarkerOverlay.vue', import.meta.url), 'utf8')
...
const sharingSectionSource = await readFile(new URL('../src/modules/strategy-planner/components/inspector/StrategySharingSection.vue', import.meta.url), 'utf8')
...
test('strategy setup, command tools, marker creation and inspector concerns remain separate', () => {
  assert.equal(toolbarSource.match(/<details\b/g)?.length, 4)
  const inspectorSectionSources = [markerSectionSource, selectionSectionSource, transformSectionSource, sharingSectionSource]
  assert.equal(inspectorSource.match(/<details\b/g)?.length || 0, 0)
  assert.equal(inspectorSectionSources.join('\n').match(/<details\b/g)?.length, 4)
  ...
  assert.match(inspectorSource, /strategy-inspector-sections/)
  assert.match(markerOverlaySource, /strategy-marker-overlay/)
  assert.match(markerOverlaySource, /StrategyMarkerSection/)
  assert.match(inspectorSource, /StrategySharingSection/)
  ...
  assert.match(plannerPageSource, /StrategyInspector/)
  ...
  assert.match(plannerPageSource, /components\/inspector\/StrategyInspector/)
  assert.match(plannerPageSource, /components\/marker-deck\/StrategyMarkerOverlay/)
  assert.match(plannerPageSource, /aria-controls="strategy-tool-rail"/)
  assert.match(inspectorSource, /strategy-inspector-sections/)
  assert.match(inspectorSource, /strategy-inspector-toggle/)
  assert.match(plannerPageSource, /markerToolsOpen/)
  assert.match(plannerPageSource, /inspectorOpen/)
})
