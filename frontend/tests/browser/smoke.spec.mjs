...
test('strategy planner keeps the chart separate and saves website-backed markers without a player', async ({ page }) => {
  ...
  await textCommand.locator('summary').click()
  const inspectorSections = page.locator('.strategy-tool-section')
  await expect(inspectorSections).toHaveCount(2)
  ...
  await expect(page.locator('.strategy-tool-rail')).toHaveCSS('overflow', 'visible')
  await expect(page.locator('.strategy-inspector-panel')).toHaveCSS('overflow', 'visible')
  await expect(page.locator('.strategy-inspector-panel')).toHaveCSS('max-height', 'none')
  ...
  const inspectorBox = await page.locator('.strategy-inspector-panel').boundingBox()
  ...
  expect(inspectorBox.width).toBeGreaterThan(workspaceBox.width * 0.95)
  ...
  await page.locator('.strategy-inspector-toggle').click()
  await expect(page.locator('.strategy-inspector-panel')).toBeVisible()
  await expect(page.locator('.strategy-inspector-control-card')).toBeVisible()
  await expect(page.locator('.strategy-inspector-sections .strategy-tool-section')).toHaveCount(0)
  await expect(page.locator('.strategy-tool-rail')).toBeVisible()
  await page.locator('.strategy-marker-overlay-close').click()
  await expect(page.locator('.strategy-tool-rail')).toBeHidden()
  await expect(page.locator('.strategy-marker-tools-toggle')).toBeVisible()
  await page.locator('.strategy-marker-tools-toggle').click()
  await expect(page.locator('.strategy-tool-rail')).toBeVisible()
  ...
  await expect(page.locator('.strategy-inspector-panel')).toBeVisible()
  await expect(page.locator('.strategy-inspector-control-card')).toBeVisible()
  await expect(page.locator('.strategy-inspector-toggle')).toContainText('Show inspector')
  await expect(page.locator('.strategy-inspector-toggle')).toHaveAttribute('aria-expanded', 'false')
  await page.locator('.strategy-inspector-toggle').click()
  await expect(page.locator('.strategy-inspector-panel')).toBeVisible()
  await expect(page.locator('.strategy-inspector-toggle')).toHaveAttribute('aria-expanded', 'true')
  await page.setViewportSize({ width: 390, height: 844 })
  await expect(page.locator('.strategy-inspector-toggle strong')).toHaveCSS('display', 'none')
