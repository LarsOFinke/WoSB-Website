...
test('strategy planner keeps the chart separate and saves website-backed markers without a player', async ({ page }) => {
  ...
  await expect(commandSections.nth(2)).toHaveAttribute('open', '')
  await commandSections.nth(2).locator('summary').click()
  await expect(commandSections.nth(2)).not.toHaveAttribute('open', '')
  await commandSections.nth(2).locator('summary').click()
  const inspectorSections = page.locator('.strategy-tool-section')
  await expect(inspectorSections).toHaveCount(3)
  await inspectorSections.nth(1).locator('summary').click()
  ...
  await page.locator('.strategy-tools-toggle').click()
  await expect(page.locator('.strategy-tool-rail')).toBeHidden()
  await expect(page.locator('.strategy-tools-toggle')).toContainText('Show tools')
  await expect(page.locator('.strategy-tools-toggle')).toHaveAttribute('aria-expanded', 'false')
  await page.locator('.strategy-tools-toggle').click()
  await expect(page.locator('.strategy-tool-rail')).toBeVisible()
  await expect(page.locator('.strategy-tools-toggle')).toHaveAttribute('aria-expanded', 'true')
  await page.setViewportSize({ width: 390, height: 844 })
  await expect(page.locator('.strategy-tools-toggle strong')).toHaveCSS('display', 'none')
