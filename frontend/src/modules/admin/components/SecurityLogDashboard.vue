...
const maxDayRequests = computed(() => Math.max(1, ...(dashboard.value.days || []).map((day) => Number(day.total || 0))))
...
function dayBarStyle(day) {
  return { width: `${Math.max(2, (Number(day.total || 0) / maxDayRequests.value) * 100)}%` }
}
    ...
    <article class="security-surface security-overview-surface">
      ...
      <section class="security-filter-console" :aria-label="t('admin.security.filtersTitle')">
        <div class="security-filter-console-head">
          <div>
            <h4>{{ t('admin.security.filtersTitle') }}</h4>
            <p>{{ t('admin.security.filtersHint') }}</p>
          </div>
        ...
        <div class="security-filter-grid">
          <div class="security-filter-group security-date-filter-group">
            ...
            <div class="security-quick-range-row">
              ...
              <button type="button" @click="setRange(30)">{{ t('admin.security.thirtyDays') }}</button>
            </div>
          ...
          <div class="security-filter-group security-threat-filter-group">
            ...
            <div class="security-threat-filter-row">
              ...
              <button
                v-for="level in threatLevels"
                :key="level"
                type="button"
                :class="[`threat-${level}`, { 'is-active': threatLevel === level }]"
                @click="setThreat(level)"
              >
            ...
            <label class="security-ip-picker">
              ...
              <select :value="selectedIp" @change="emit('update:selectedIp', $event.target.value)">
                ...
                <option v-for="row in ipOptions" :key="row.client_ip" :value="row.client_ip">
                  {{ row.client_ip }} · {{ t(`admin.security.levels.${row.threat_level}`) }} {{ row.threat_score }}/100 · {{ row.request_count }}
                </option>
              </select>
            </label>
      ...
      <div class="security-metric-grid">
        ...
        <article><span>{{ t('admin.security.requests') }}</span><strong>{{ dashboard.total_requests }}</strong><small>{{ t('admin.security.filteredView') }}</small></article>
        ...
        <article><span>{{ t('admin.security.suspicious') }}</span><strong>{{ dashboard.suspicious_hits }}</strong><small>Probe / scan</small></article>
        <article><span>4xx</span><strong>{{ dashboard.status_4xx }}</strong><small>Client</small></article>
        <article><span>5xx</span><strong>{{ dashboard.status_5xx }}</strong><small>Server</small></article>
      </div>
      ...
      <div class="security-analysis-grid">
        <section class="security-table-card security-day-card">
          <div class="security-card-head">
            <div>
              ...
              <p class="muted">{{ t('admin.security.clickDayHint') }}</p>
            </div>
          </div>
              ...
              <tbody>
                <tr v-for="day in dashboard.days" :key="day.day" tabindex="0" @click="setExactDay(day.day)" @keydown.enter="setExactDay(day.day)">
                  ...
                  <td>
                    ...
                    <span class="security-day-bar"><i :style="dayBarStyle(day)"></i></span>
                  </td>
        ...
        <section class="security-table-card security-focus-card">
          <div class="security-card-head">
            <div>
              <h4>{{ selectedIp ? t('admin.security.selectedIp') : t('admin.security.topMatches') }}</h4>
              <p class="muted">{{ selectedIp ? t('admin.security.filteredView') : t('admin.security.chooseIpHint') }}</p>
            </div>
          </div>
          ...
          <div v-else-if="topIpRows.length" class="security-top-ip-list">
            <button v-for="row in topIpRows" :key="row.client_ip" type="button" @click="emit('update:selectedIp', row.client_ip)">
              ...
              <span class="security-top-ip-copy"><strong>{{ row.client_ip }}</strong><small>{{ row.request_count }} {{ t('admin.security.requests').toLowerCase() }} · {{ row.suspicious_hits }} {{ t('admin.security.suspicious').toLowerCase() }}</small></span>
