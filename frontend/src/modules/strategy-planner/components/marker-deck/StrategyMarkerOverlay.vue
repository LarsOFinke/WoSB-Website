...
import StrategyMarkerSection from './StrategyMarkerSection.vue'
...
<template>
  <aside id="strategy-tool-rail" class="strategy-tool-rail strategy-marker-overlay" :aria-label="t('strategyPlanner.marker')">
    <header class="strategy-marker-overlay-head">
      <div>
        <span class="strategy-tools-kicker">{{ t('strategyPlanner.marker') }}</span>
        <strong>{{ t('strategyPlanner.addMarker') }}</strong>
      </div>
      <button type="button" class="strategy-marker-overlay-close" :aria-label="t('strategyPlanner.hideMarkerTools')" @click="emit('close')">
