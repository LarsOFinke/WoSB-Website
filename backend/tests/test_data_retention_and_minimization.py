...
def test_retention_policy_removes_only_expired_operational_and_personal_records() -> None:
    ...
    with TestClient(app), SessionLocal() as db:
        ...
        old_log = SecuritySignalBucket(
            day=(now - timedelta(days=8)).date(),
            client_ip="198.51.100.30",
            signal=SECURITY_SIGNAL_RECONNAISSANCE,
            event_count=3,
        )
        fresh_log = SecuritySignalBucket(
        ...
        fresh_pending = RegistrationRequest(
            ...
            display_name="Fresh pending",
            status=REGISTRATION_PENDING,
            created_at=now - timedelta(days=1),
            updated_at=now,
        )
        old_reviewed = RegistrationRequest(
            username=f"{prefix}-reviewed",
            password_hash=REDACTED_REGISTRATION_PASSWORD_HASH,
            display_name="Old reviewed",
        ...
        ids = {
            "old_log": old_log.id,
            "fresh_log": fresh_log.id,
            "inactive_block": inactive_block.id,
            "old_audit": old_audit.id,
            "fresh_audit": fresh_audit.id,
            "old_delivery": old_delivery.id,
            "fresh_delivery": fresh_delivery.id,
            "old_consent": old_consent.id,
            "fresh_consent": fresh_consent.id,
