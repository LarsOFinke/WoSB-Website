...
def _load_required_env_file() -> dict[str, str]:
    env_path = Path(
        os.environ.get("RBF_ENV_FILE", os.environ.get("RBV_ENV_FILE", os.environ.get("BLACKWATER_ENV_FILE", os.environ.get("WOSB_ENV_FILE", DEFAULT_ENV_FILE))))
    ).expanduser()
...
def _load_required_cfg() -> dict[str, Any]:
    cfg_path = Path(
        os.environ.get("RBF_CONFIG_FILE", os.environ.get("RBV_CONFIG_FILE", os.environ.get("BLACKWATER_CONFIG_FILE", os.environ.get("WOSB_CONFIG_FILE", DEFAULT_CFG_FILE))))
    ).expanduser()
    if not cfg_path.exists():
        raise ConfigError(
            f"Missing required config file: {cfg_path}. "
            "Keep backend/config/app.toml in the deployment or set RBF_CONFIG_FILE."
        )
