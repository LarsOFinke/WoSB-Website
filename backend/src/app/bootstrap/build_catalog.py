...
def seed_build_option_catalog(db: Session) -> None:
    ...
    option_groups = tuple(
        [row.model_dump(mode="json") for row in document.items]
    ...
    _validate_catalog(category_rows, option_groups)
    categories = _seed_categories(db, category_rows)
    _migrate_legacy_upgrade_names(db, categories["upgrade"], aliases)
    _seed_options(db, categories, option_groups)
    db.commit()


def _validate_catalog(
    category_rows: list[dict[str, object]],
    option_groups: tuple[list[dict[str, object]], ...],
) -> None:
    by_category = {
        str(group[0]["category"]): group
        for group in option_groups
        if group
    }
    validate_build_option_catalog(category_rows, option_groups)
    validate_sail_seed_data(by_category["sail"])
    validate_upgrade_seed_data(by_category["upgrade"])
    validate_lantern_seed_data(by_category["lantern"])
    validate_weapon_seed_data(by_category["weapon"])
    validate_special_crew_seed_data(by_category["special_crew"])


def _seed_categories(
    db: Session, category_rows: list[dict[str, object]]
) -> dict[str, BuildItemCategory]:
