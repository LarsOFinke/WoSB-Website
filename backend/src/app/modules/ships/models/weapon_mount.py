...
class ShipWeaponMount(Base):
    ...
    option_allowances: Mapped[list["ShipWeaponOptionAllowance"]] = relationship(
...
class ShipWeaponOptionAllowance(Base):
    """Explicit audited exception to the regular mount-size ceiling.

    Most weapons are selected through the normalized Light/Medium/Heavy mount
    ceiling. A small number of named bow/stern assemblies are compatible with
    specific ship mounts despite that general ceiling. Those exceptions belong
    in a join table instead of hard-coded ship or weapon names.
    """

    __tablename__ = "ship_weapon_option_allowances"
    __table_args__ = (
        UniqueConstraint(
    ...
    id: Mapped[int] = mapped_column(Integer, primary_key=True)
    ship_weapon_mount_id: Mapped[int] = mapped_column(
        ForeignKey("ship_weapon_mounts.id", ondelete="CASCADE"),
        nullable=False,
        index=True,
    )
    build_item_option_id: Mapped[int] = mapped_column(
        ForeignKey("build_item_options.id", ondelete="CASCADE"),
        nullable=False,
        index=True,
    )

    mount: Mapped[ShipWeaponMount] = relationship(
        ShipWeaponMount, back_populates="option_allowances"
    )
