...
def build_management_context(db: Session, actor: User, fleet_id: int) -> FleetManagementContext:
    ...
    return FleetManagementContext(
        actor=actor,
        fleet_id=fleet_id,
        actor_fleet_role=actor_membership.role if actor_membership else None,
...
def membership_permissions(
    ...
    if actor.is_admin:
        ...
        return _base_permissions(
            assignable_roles=(FLEET_ROLE_MEMBER, FLEET_ROLE_LIEUTENANT, FLEET_ROLE_ADMIRAL),
    ...
    if actor.role == ROLE_MODERATOR:
        return _base_permissions(
            assignable_roles=(FLEET_ROLE_MEMBER, FLEET_ROLE_LIEUTENANT),
            can_edit_directory=True,
            can_change_role=True,
            can_change_status=True,
        )

    if context.actor_fleet_role == FLEET_ROLE_ADMIRAL:
        return _base_permissions(
            assignable_roles=(FLEET_ROLE_MEMBER, FLEET_ROLE_LIEUTENANT),
            can_edit_directory=True,
            can_change_role=True,
    ...
    if context.actor_fleet_role == FLEET_ROLE_LIEUTENANT:
        if target.role != FLEET_ROLE_MEMBER:
            return _base_permissions(reason="fleet_peer")
        return _base_permissions(
            assignable_roles=(FLEET_ROLE_MEMBER,),
            can_edit_directory=True,
