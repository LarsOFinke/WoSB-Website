...
MODE="${1:-generated}"
[[ "$MODE" == generated || "$MODE" == --all ]] || { echo 'Usage: scripts/clean_repository.sh [--all]' >&2; exit 2; }
...
find "$ROOT_DIR" -type d \( -name .git -o -name node_modules \) -prune -o -type f \( -name '*.pyc' -o -name '*.pyo' -o -name '.coverage*' \) -delete
