...
- Fixed Raid-Helper event creation against the documented v4 API: profiles now use and migrate to `raid-helper.xyz`, exact JSON placeholders preserve identifier types, and bounded top-level API validation messages are shown to event managers instead of a bare HTTP status. Failed deliveries now expose an explicit manager-only retry action. New and unchanged default templates receive the safe advanced time-display and signup settings (`date_variant=both`, 24-hour zoned time, long info layout, preserved signup order, and unregister support), while custom payloads remain untouched.
