...
- Corrected normal bow/stern weapon compatibility: Zeus, Basilisk, Poseidon and every other `bow_stern` weapon are now available to every ship with an armed bow or stern mount through normalized option-to-slot relationships. Removed the mistaken per-ship allowance table; Light/Medium/Heavy ceilings remain exclusive to broadside cannons, while mortars and special weapons keep their dedicated rules. Migration `0010` removes the obsolete exception rows and clears inapplicable weapon classes from positional weapons.
